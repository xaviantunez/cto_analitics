...
$(document).ready(function() {
    ...
    function guardar(){
        ...
        for (var i = 0; i < nombresCronometros.length; i++) {
            ...
            if ($("#check" + jugador).is(':checked')) {
                ...
                $("#SP"+jugador).show();
                if(texto=="") texto="Equipo en el campo: "+jugador.replace(/_/g, " ")
                else texto+=", "+jugador+' '
        ...
        if(enmarcha==0 && (guardado=="")){
            mensajeTablaMarcador(texto);
            guardado=1;
            storageManager("guardar",'guardado', 1);
        }
    }
    function iniciar(){
        var texto=""
    ...
    function removeestadisticas() {
        arrayestadisticas.forEach(function (estadistica) {
            storageManager("borrar",estadistica+'left');
            storageManager("borrar",estadistica+'right');
        });
    ...
    function reiniciarIntervalos() {
        ...
        storageManager("borrar",'checkeados');
        storageManager("borrar",'tiempos');
        storageManager("borrar",'tiempototal');
        storageManager("borrar",'adversario');
        storageManager("borrar",'equipoelegido');
        storageManager("borrar",'resulCto');
        storageManager("borrar",'resulAdv');
        storageManager("borrar",'capitan');
        storageManager("borrar",'iniciado');
        storageManager("borrar",'marcatiempo');
        storageManager("borrar",'marcamensajes');
        storageManager("borrar",'mensajes');
        storageManager("borrar",'marcatiempototal');
        storageManager("borrar",'marcatiempototaldate');
        storageManager("borrar",'mensajestablamarcador');
        storageManager("borrar",'portero');
        storageManager("borrar",'expulsados');
        storageManager("borrar",'rematesFueraRight');
        storageManager("borrar",'guardado');
        ...
        $("#cronometros").empty();
        storageManager("borrar",'nombresCronometros');
        storageManager("borrar",'equipotitular');
    ...
    function add_fila_player(nombresCronometros,i){
        ...
        checkboxp.on("click",function(e){
            checkPortero(nombresCronometros[i])});
    ...
    function checkPortero(nombre) {
        for (var i = 0; i < nombresCronometros.length; i++) {
            var jugador = nombresCronometros[i];
            console.log("check "+jugador+' '+$("#pcheck" + jugador).is(':checked'));
            if(nombre==jugador && $("#pcheck" + jugador).is(':checked') ){
                $("#pcheck" + jugador).prop("checked", true);
    ...
    $('.cronometro').click(function() {
        ...
        if(maxTiempo>60) $("#spbtn3"+this.id).toggle();
